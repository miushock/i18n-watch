module.exports = watch;

var Git = require("nodegit");

var fse = require('fs-extra');
var path = require('path');
var nodegit = require('nodegit');
var sh = require('shelljs');

function watch(args) {

    let repo_url = args[0];
    // let repository;
    // 获取命令行参数
    const parse_options = require('./common/parse_options.js');
    // 获取其它参数 并转化为对象
    let options = parse_options(args.slice(1));
    // 获取 tracklist 类
    let TrackList = require('./common/tracklist.js').TrackList;
    let tracklist = new TrackList();

    // 目录 --dir
    let repo_dir_tail = options['dir'] || repo_url.split('/').slice(-1)[0];
    // 组合出镜像下载目录
    let repo_dir = path.resolve(process.cwd(), repo_dir_tail);
    //
    let i18n_path = options['i18n-path'] || './config/locale';

    // 追加 tracklist.json 配置文件
    let tracklist_item_key = repo_dir_tail;
    let tracklist_item_val = {
        "repo": repo_url,
        "i18n_path": i18n_path
    };
    tracklist.add(tracklist_item_key, tracklist_item_val);

    let sparse_co_config = path.resolve(repo_dir, '.git/info/sparse-checkout');

    fse.pathExists(repo_dir)
        //shallow clone
        .then(exists => {
            if (exists) {
                throw new Error('target dir already exist!')
            }
            // if (sh.exec(`git clone --depth=1 ${repo_url} ${repo_dir}`).code !== 0) {
            //     throw new Error('clone failed!')
            // }
            // var opts = {
            //     checkoutBranch: 'test',
            //     fetchOpts: {
            //         callbacks: {
            //             credentials: function () {
            //                 // const account = 'liu-kai-github';
            //                 // const password = 'LiuShuojia2378';
            //                 // works
            //                 // return Git.Cred.userpassPlaintextNew(account, password)
            //                 //doesn't work
            //                 return Git.Cred.sshKeyNew( '688faaab1e6340f9e14b0c94ec9224b625f55a41', "x-oauth-basic");
            //             },
            //             certificateCheck: function () {
            //                 return 1;
            //             }
            //         }
            //     }
            // };
            // // Git.Clone("https://github.com/nodegit/nodegit", "./tmp")
            // return Git.Clone(repo_url, repo_dir, opts);
            // return nodegit.Repository.open(repo_dir)
            const cloneRepository = require('./server/cloneRepository');
            return cloneRepository(repo_url, repo_dir,'test', '688faaab1e6340f9e14b0c94ec9224b625f55a41');
        })

        // .then((repository) => {
        //     const checkRemoteBranch = require('./server/checkRemoteBranch');
        //     return checkRemoteBranch(repository, 'test');
        // })
        // .then((reference) => {
        //     return console.log(reference, 'reference');
        // })


        // //add source back to remote list
        // .then(repository => {
        //     // repository = repo;
        //     // return nodegit.Remote.create(repo, 'upstream', repo_url);
        //     return repository.checkoutBranch('dev', {});
        // })
        //
        // //prepare for sparse checkout
        // .then(() => {
        //     return repository.config();
        // })
        // .then(config => {
        //     config.setBool('core.sparsecheckout', 1);
        // })
        // .then(() => {
        //     fse.ensureFileSync(sparse_co_config);
        //     return fse.writeFile(sparse_co_config, 'config/locale/*');
        // })
        //
        // //create and switch to i18n branch
        // .then(() => {
        //     return repository.getHeadCommit();
        //
        // })
        // .then(commit => {
        //     return repository.createBranch(
        //         'i18n',
        //         commit,
        //         0
        //     );
        // })
        // .then(ref => {
        //     sh.cd(repo_dir);
        //     if (sh.exec('git checkout i18n').code !== 0) {
        //         throw new Error('checkout failed!');
        //     }
        // })
        .catch(error => {
            fse.remove(repo_dir);
            tracklist.remove(repo_dir_tail);
            console.log(error);
        });
}
